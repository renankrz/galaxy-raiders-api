...
data class SpaceField(val width: Int, val height: Int, val generator: RandomGenerator) {
  ...
  fun generateExplosion(x: Double, y: Double) {
    this.explosions += this.createExplosion(x, y)
  }

  fun removeExplosion(e: Explosion) {
    this.explosions -= e
  }
  ...
  private fun createExplosion(x: Double, y: Double): Explosion {
    return Explosion(
      initialPosition = Point2D(x, y)
    )
  }
}
